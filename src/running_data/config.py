...
haben (siehe Hinweis bei :data:`RAW_CORE_COLUMNS`).
"""
...
#: Enthält ``duration`` als Rohtext (Garmin: "hh:mm:ss", Apple: Minuten).
#:
#: Hinweis: Diese Liste und :data:`CORE_COLUMNS` unterschieden sich im
#: Notebook nur durch ``duration`` vs. ``duration_sec``, waren aber als zwei
#: unabhängige Listen gepflegt. In der Folge behielt der Garmin-Zweig die
#: Rohspalte ``duration`` bis ins Endergebnis, während sie bei Apple wegfiel —
#: die Ursache der leeren ``duration``-Werte bei Apple-Zeilen (TODO 11).
#: Das Verhalten bleibt in diesem Refactoring bewusst unverändert; die
#: Konstanten machen die Abweichung lediglich sichtbar.
