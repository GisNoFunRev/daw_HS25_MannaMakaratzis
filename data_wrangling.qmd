...
Wiederverwendbare Validierungsfunktionen für konsistente Datenqualitätsprüfung über alle Quellen hinweg.
...
Robuste Cleaning Pipeline mit strukturierter Fehlerbehandlung und Schritt-Dokumentation.
...
Einzelne, wiederverwendbare Funktionen für jeden Bereinigungsschritt.
...
def step_validate_plausibility(
    ...
    """Applies all plausibility checks"""
    validator = DataValidator()

    mask = (
        validator.validate_distance(df, config)
        & validator.validate_duration(df, config)
        & validator.validate_pace(df, config)
    )

    removed_reasons = {
        "distance": (~validator.validate_distance(df, config)).sum(),
        "duration": (~validator.validate_duration(df, config)).sum(),
        "pace": (~validator.validate_pace(df, config)).sum(),
    }
